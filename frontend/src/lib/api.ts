...
export type TapStatus = 'none' | 'pending_sent' | 'pending_received' | 'tapped';
...
export const api = {
  ...
  users: {
    profile: (id: number, token: string) =>
      request<UserProfile>(`/users/${id}`, {}, token),
    me: (token: string) => request<User>('/users/me/profile', {}, token),
    tap: (id: number, token: string) =>
      request<{ tapStatus: TapStatus }>(`/users/${id}/tap`, { method: 'POST' }, token),
  },
};
